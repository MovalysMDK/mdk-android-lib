...
public class MMDatePickerDialogFragment extends MMDialogFragment implements OnClickListener, OnDateChangedListener, OnDismissListener {
	...
	public void onCreate(Bundle p_oSavedInstanceState) {
		...
		if(getArguments() != null && oCurrentDateValue == null) {
			...
			this.minValue = getArguments().getLong(ConfigurableVisualComponent.Attribute.MIN_DATE_ATTRIBUTE.getName());
			this.maxValue = getArguments().getLong(ConfigurableVisualComponent.Attribute.MAX_DATE_ATTRIBUTE.getName());
		}
	}
	...
	public Dialog onCreateDialog(Bundle p_oSavedInstanceState) {
        ...
        {
			if (this.minValue != 0) {
				uiDatePicker.setMinDate(this.minValue);
			}
			if (this.maxValue != 0) {
				uiDatePicker.setMaxDate(this.maxValue);
			}
        }
