...
public class MMSearchSpinnerDialogFragment extends MMDialogFragment implements OnDismissListener{
	...
	public View onCreateView(LayoutInflater p_oInflater, ViewGroup p_oContainer,
		...
		filter = (EditText) oContentView.findViewById(application.getAndroidIdByRKey(AndroidApplicationR.filtre_section));
		
		this.filter.addTextChangedListener((MMSearchSpinner)getDismissListener());
		this.itemList.setAdapter(new ConfigurableSpinnerListAdapter(((MMSearchSpinner)getDismissListener()).getAdapter().getAdapter(), mItemVmInterface, ((MMSearchSpinner)getDismissListener()), ((MMSearchSpinner)getDismissListener()).hasEmptyValue()));
	...
	public <ITEM extends MIdentifiable, ITEMVM extends ItemViewModel<ITEM>, LISTVM extends ListViewModel<ITEM, ITEMVM>> void filter(ListViewModelFilter<ITEM,ITEMVM,String> p_oFilter) {
		((ConfigurableSpinnerListAdapter<ITEM,ITEMVM, LISTVM>) this.itemList.getAdapter()).getAdapter().getMasterVM().filter(p_oFilter, this.filter.getText().toString());
