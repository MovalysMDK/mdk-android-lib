...
public abstract class AbstractWorkspaceMasterDetailMMFragmentActivity extends AbstractBaseWorkspaceMMFragmentActivity<MMWorkspaceMasterDetailFragmentLayout> implements OnTabChangeListener {
	...
	/** loose change listener */
	private OnClickListener looseListener;
	/** keep change listener */
	private OnClickListener keepListener;
	/** cancel change listener */
	...
	public void doOnMasterListChangeSelectedItem(final WorkspaceListHandler p_oHandler, final BusinessEvent<MMPerformItemClickEventData> p_oEvent) {
		...
		if ( p_oHandler != null ) {
			...
			if ( this.isViewModelModified() && ( sNextItemId == null || !sNextItemId.equals(this.selectedItemId) ) ) {
				this.createListeners(p_oEvent, sNextItemId, p_oHandler);
				this.doOnViewModelModified(
						this.looseListener
						/*new DialogInterface.OnClickListener() {
							@Override
							public void onClick(DialogInterface p_oDialog, int p_iId) {
								// this reacts to the click on the "yes" button of the confirmation dialog
								p_oDialog.cancel();
								if ( p_oEvent.getData() != null ) {
									p_oEvent.getData().performItemClick();
								}						
						...
						/*new DialogInterface.OnClickListener() {
							@Override
							public void onClick(DialogInterface p_oDialog, int p_iId) {
								// this reacts to the click on the "no" button of the confirmation dialog
								p_oDialog.cancel();
								if ( p_oEvent.getData() != null ) {
									p_oEvent.getData().performItemClick();								
								}								
						...
						/*new DialogInterface.OnClickListener() {
							@Override
							public void onClick(DialogInterface p_oDialog, int p_iId) {
								// this reacts to the click on the "cancel" button of the confirmation dialog
								p_oDialog.cancel();
								AbstractWorkspaceMasterDetailMMFragmentActivity.this.doDisplayDetail();
							}
						}*/);
	...
	/**
	 * Create listener in case of change and item click
	 * @param p_oEvent the event that trigger after change
	 * @param p_sNextItemId the id of the next item clicked
	 * @param p_oHandler the workspace list handler
	 */
	private void createListeners(final BusinessEvent<MMPerformItemClickEventData> p_oEvent, final String p_sNextItemId, final WorkspaceListHandler p_oHandler) {
		this.looseListener = new DialogInterface.OnClickListener() {
			@Override
			public void onClick(DialogInterface p_oDialog, int p_iId) {
				// this reacts to the click on the "yes" button of the confirmation dialog
				p_oDialog.cancel();
				...
				AbstractWorkspaceMasterDetailMMFragmentActivity.this.selectedItemId = p_sNextItemId;
				AbstractWorkspaceMasterDetailMMFragmentActivity.this.doOnKeepWorkspaceModifications(
						(View) p_oHandler.getView());
				AbstractWorkspaceMasterDetailMMFragmentActivity.this.doDisplayDetail();
			}
		...
		this.keepListener = new DialogInterface.OnClickListener() {
			@Override
			public void onClick(DialogInterface p_oDialog, int p_iId) {
				// this reacts to the click on the "no" button of the confirmation dialog
				p_oDialog.cancel();
		...
		this.cancelListener = new DialogInterface.OnClickListener() {
			@Override
			public void onClick(DialogInterface p_oDialog, int p_iId) {
				// this reacts to the click on the "cancel" button of the confirmation dialog
				AbstractWorkspaceMasterDetailMMFragmentActivity.this.goToPage = 1;
				AbstractWorkspaceMasterDetailMMFragmentActivity.this.doChangePage();
			}
		};
	}
