...
public class ApplicationMainImpl extends AbstractMMActivity implements ApplicationMain { 
	...
	private MMTextView oUiTextVersion;
	/** 
	...
	protected void initUIVersionNumber() {
		oUiTextVersion=(MMTextView)this.findViewById(this.getAndroidApplication().getAndroidIdByRKey(AndroidApplicationR.screen_splash_label));
		String sVersion;
		try {
	...
	protected void doOnSynchroFinish(SynchronizationActionParameterOUT p_oResult) {
		...
		if (p_oResult == null 
			...
			final FwkController oController = Application.getInstance().getController(); 
			if (oController.isPushActivated()) {
				this.launchAction(ActivatePushAction.class, null);
			}
			else {
				startMainScreen();
			}
	 ...
	 */
	@ListenerOnActionSuccess(action = ActivatePushAction.class)
	public void doOnActivitePushAction(ListenerOnActionSuccessEvent<NullActionParameterImpl> p_oEvent) {
		startMainScreen();
	}
