 ...
 *
 */
public class MMMultiSectionLayout extends LinearLayout implements ConfigurableLayoutComponent<NoneType>, MMIdentifiableView {
	...
	private static final String ATTR_INIT_CONTAINER = "initContainer";

	private static final boolean DEFAULT_INIT_CONTAINER = true;
	 ...
	 * configuration
	 */
	private ManagementConfiguration manager ;
	...
	/**
	 * Indicates if we a use the root container
	 */
	private boolean initRootContainer = true;
	 ...
	 */
	public MMMultiSectionLayout(Context p_oContext) {
		super(p_oContext);
		if(!isInEditMode()) {
			this.aivDelegate = new AndroidConfigurableLayoutComponentDelegate<>(this);
			this.setInitRootContainer(DEFAULT_INIT_CONTAINER);
			this.init();
		}
	}
	 ...
	 * @see LinearLayout#LinearLayout(Context)
	 */
	public MMMultiSectionLayout(Context p_oContext, AttributeSet p_oAttrs) {
	...
	protected final void initFromAttributes(AttributeSet p_oAttrs) {
		this.setInitRootContainer(p_oAttrs.getAttributeBooleanValue(Application.MOVALYSXMLNS, ATTR_INIT_CONTAINER, DEFAULT_INIT_CONTAINER));
	}
	...
	public final void init() {
		...
		if (this.initRootContainer) {
			LayoutInflater oInflater = LayoutInflater.from(this.getContext());

			if ( this.rootContainer != null && this.rootContainer.getParent() != null ) {
				((ViewGroup) this.rootContainer.getParent()).removeView( this.rootContainer);
			}

			this.computeSectionContainer(oInflater, this.manager.getVisibleGroups().get(0));

			this.application.computeId(this.rootContainer, SECTION_CONTAINER_NAME);
			this.addView( this.rootContainer);
		}
	}

	protected final void setInitRootContainer(final boolean p_bInitRootContainer) {
		this.initRootContainer = p_bInitRootContainer;
	}
	 ...
	 * @param p_oGroup
	 */
	protected void computeSectionContainer(LayoutInflater p_oInflater, ManagementGroup p_oGroup) {
		View oLayoutZone;
		if (this.rootContainer ==null) {
			this.rootContainer = this.createContainer();
			this.application.computeId(this.rootContainer, p_oGroup.getName());
			...
			for (ManagementZone oZone : listZones) {
				...
				this.sectionContainer.addView(oLayoutZone);
			}
		}
		else {
			// il faut retailler le pannel
			this.rootContainer.setLayoutParams(new LinearLayout.LayoutParams(LinearLayout.LayoutParams.MATCH_PARENT, LinearLayout.LayoutParams.WRAP_CONTENT));
		}
	...
	protected ViewGroup createContainer() {
		...
		MMLinearLayout oSectionContainer = new MMLinearLayout(this.getContext());
		...
		oSectionContainer.setLayoutParams(oParamsLayout);
		oSectionContainer.setOrientation(VERTICAL);

		MMScrollView oScroll = new MMScrollView(this.getContext());
		oScroll.setBackgroundColor(android.R.color.transparent);
		oScroll.setLayoutParams(new LayoutParams(LayoutParams.MATCH_PARENT, LayoutParams.MATCH_PARENT));
		oScroll.setHorizontalScrollBarEnabled(false);
		oScroll.setVerticalScrollBarEnabled(true);
		oScroll.addView(oSectionContainer);

		this.sectionContainer = oSectionContainer ;
