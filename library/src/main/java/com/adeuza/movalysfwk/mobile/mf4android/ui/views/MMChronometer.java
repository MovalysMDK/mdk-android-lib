...
/**
 * <p>Chronometer widget used in the Movalys Mobile product for Android</p>
 * <p>Copyright (c) 2010</p>
 * <p>Company: Adeuza</p>
 */
public class MMChronometer extends TextView implements
ConfigurableVisualComponent<ChronometerParameterVM>, MMIdentifiableView, InstanceStatable {

	/** configurable view delegate */
	private AndroidConfigurableVisualComponentDelegate<ChronometerParameterVM> aivDelegate = null;
	/**
	 * View model du chronometre
	 */
	private ChronometerParameterVM value = null;
	 ...
	 * @param p_oContext
	 *            the context to use
	 * @see Chronometer#Chronometer(Context)
	 */
	 ...
	 * {@inheritDoc}
	 * 
	 * @see com.adeuza.movalysfwk.mobile.mf4android.ui.views.MMValueableView#setValue(java.lang.Object)
	 */
	@Override
	public void configurationSetValue(ChronometerParameterVM p_oObjectToSet) {
		...
		if (p_oObjectToSet != null) {
			...
			lCurrentValue = this.value.initValue;
			if (this.task != null) {
				this.task.cancel(true);
			}
			this.task = new MMChronometerTask();
			if (p_oObjectToSet.start) {
				lCurrentValue = this.value.initValue
						+ (System.currentTimeMillis() - this.value.lastStart);
				((AndroidApplication) Application.getInstance()).execAsyncTask(this.task);
			}
	...
	public void updateVisual() {
		long lCurrentValue = this.value.initValue
				+ (System.currentTimeMillis() - this.value.lastStart);
		this.setText(DurationUtils.getInstance().format(lCurrentValue /  DurationUtils.MILLISECONDS_IN_ONE_SECOND ));
	}
	 ...
	 */
	@Override
	public boolean isNullOrEmptyValue(ChronometerParameterVM p_oObject) {
	...
	public void doOnPostAutoBind() {
		if (this.task != null && this.value != null && this.value.start) {
			// this is normally not appening
			throw new UnsupportedOperationException("may do somesing about that");
		}
