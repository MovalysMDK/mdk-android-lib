...
public class MMBooleanImageView extends ImageView implements ConfigurableVisualComponent<Boolean>, MMIdentifiableView, InstanceStatable {
	 ...
	 */
	public MMBooleanImageView(Context p_oContext) {
		super(p_oContext);
		if(!isInEditMode()) {
	...
	public void configurationSetValue(Boolean p_oObjectToSet) {
		...
		if ( p_oObjectToSet == null ) {
			p_oObjectToSet = Boolean.FALSE ;
		}
		String sIdentifier = this.imageNamePrefix + p_oObjectToSet.toString();
	...
	public boolean isNullOrEmptyValue(Boolean p_oObject) {
		return p_oObject == null || !this.isMissingImageReplacedByText
				&& NO_IMAGE == ((AndroidApplication)AndroidApplication.getInstance()).getAndroidIdByStringKey(
						ApplicationRGroup.DRAWABLE, this.imageNamePrefix + p_oObject.toString() );
	}
