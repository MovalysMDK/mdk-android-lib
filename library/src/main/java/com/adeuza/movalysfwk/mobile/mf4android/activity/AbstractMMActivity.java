...
public abstract class AbstractMMActivity extends LifecycleDispatchActionBarActivity implements Screen , Comparable<Screen>, ListenerDelegator, ListenerIdentifier {
	...
	/** le lanceur d'action courrant */
	protected MMActionTask<?, ?, ?, ?> currentActionTask ;
	...
	protected void onCreate(Bundle p_oSavedInstanceState) {
		...
		//Allows to show and hide the keyboard more quickly when the view is big.
		...
		if (this.getLastCustomNonConfigurationInstance()!=null) {
			this.currentActionTask = (MMActionTask<?, ?, ?, ?>) ((Map<String,Object>) this.getLastCustomNonConfigurationInstance()).get("currentActionTask");
			//dans le cas de la rotation le dialog a disparu, il faut le reconstruire
			if (this.currentActionTask!=null && !Status.FINISHED.equals(((AsyncTask<?, ?, ?>)this.currentActionTask).getStatus())) {
				this.currentActionTask.refresh(this);
			}
		}
	...
	protected void onStop() {
		this.getAndroidApplication().removeActiveDisplayFromList(this);
		super.onStop();
	}
	...
	public Map<String,Object> onRetainCustomNonConfigurationInstance() {
		Map<String,Object> r_oMap = new HashMap<>();

		if (this.currentActionTask!=null && ActionASyncTask.class.isAssignableFrom(this.currentActionTask.getClass())) {
			((ActionASyncTask<?, ?, ?, ?>) this.currentActionTask).clear();
		}

		r_oMap.put("currentActionTask", this.currentActionTask);
		return r_oMap;
	}
	...
	/**
	 * Affecte l'objet currentActionTask 
	 * @param p_oCurrentActionTask Objet currentActionTask
	 */
	public void setCurrentActionTask(MMActionTask<?, ?, ?, ?> p_oCurrentActionTask) {
		this.currentActionTask = p_oCurrentActionTask;
	}
	/**
	...
	public boolean onCreateOptionsMenu(Menu p_oMenu) {
		if (!Application.getInstance().hasUndefinedMandatorySetting()) {
	...
	/**
	 * Permet de savoir si le screen est un panel de workspace ou non.
	 * @return true c'est un PAnel, false sinon
	 */
	public boolean isPanel() {
		return this.screenDelegate.isPanel();
	}
	 ...
	 * @param p_oIntent intent
	 * @param p_oComponentId component ID
	 * @param p_oFragmentTag fragment tag
	 * @param p_iRequestCode request code
	 */
	public void startActivityForResult(Intent p_oIntent, int p_oComponentId, String p_oFragmentTag, int p_iRequestCode) {
		int[] iValues= {p_oComponentId, p_iRequestCode};	

		registerComplexeComponent((MMComplexeComponent) findComponent(p_oComponentId, p_oFragmentTag));

		int iKey=Math.abs(computeKey(p_oComponentId, p_iRequestCode)) & ACTIVITY_SUPPORT_MASK;
	 ...
	 */
	@Override
	protected void onActivityResult(int p_oRequestCode, int p_oResultCode,
			Intent p_oData) {
	 ...
	 * @see com.adeuza.movalys.fwk.mobile.javacommons.application.ApplicationMain#loadSettings()
	 */
	 ...
	 * {@inheritDoc}
	 * NE DEVRAIT PAS ETRE ICI NE PAS SURCHARGER, NE PAS UTILISER AUTRE QUE LE FWK
	 * @see com.adeuza.movalys.fwk.mobile.javacommons.application.ApplicationMain#analyseSettings()
	 */
	...
	public void launchAction(Class<? extends Action<?,?,?,?>> p_oActionClass, ActionParameter p_oParameterIn, Object p_oSource) {
		this.currentActionTask = ((FwkControllerImpl)Application.getInstance().getController()).launchActionByActionTask(this, p_oActionClass, p_oParameterIn, p_oSource);
	}
	...
	public void launchAction(Action<?,?,?,?> p_oAction, Class<? extends Action<?,?,?,?>> p_oInterfaceClass, 
		...
		this.currentActionTask = ((FwkControllerImpl)Application.getInstance().getController()).launchActionByActionTask(
	...
	private void registerComplexeComponent(MMComplexeComponent p_oComplexeComponent) {
		...
		if(!bIsComplexeComponentAlreadyRegistered) {
			...
			oComplexeComponentDescriptor.put(MMComplexeComponent.COMPLEXE_COMPONENT_FRAGMENT_TAG_KEY, ((ConfigurableVisualComponent<?>)p_oComplexeComponent).getFragmentTag());
