...
public abstract class NumberConverter extends AbstractConverter {
	...
	private static final String TAG = NumberConverter.class.getName();
	...
	protected String convertToString(Object value) throws Throwable {
        ...
        if (useLocaleFormat && value instanceof Number) {
            ...
            if (log.isDebugLevel()) {
                log.debug(TAG, "    Converted  to String using format '" + result + "'");
            }

        } else {
            result = value.toString();
            if (log.isDebugLevel()) {
                log.debug(TAG, "    Converted  to String using toString() '" + result + "'");
            }
        }
        return result;
	...
	protected Object convertToType(Class targetType, Object value) throws Throwable {
        ...
        if (useLocaleFormat) {
            ...
            if (log.isDebugLevel()) {
                log.debug(TAG, "    No NumberFormat, using default conversion");
            }
    ...
    private NumberFormat getFormat() {
        ...
        if (pattern != null) {
            if (locale == null) {
                if (log.isDebugLevel()) {
                    log.debug(TAG, "    Using pattern '" + pattern + "'");
                }
            ...
            if (locale == null) {
                if (log.isDebugLevel()) {
                    log.debug(TAG, "    Using default Locale format");
                }
                format = NumberFormat.getInstance();
            } else {
                if (log.isDebugLevel()) {
                    log.debug(TAG, "    Using Locale[" + locale + "] format");
                }
