...
//    private static final String PACKAGE = AbstractConverter.class.getPackage().getName() + ".";
     ...
     */
    private transient Logger log = Application.getInstance().getLog();
	...
	public Object convert(Class type, Object value) {
        ...
        try {
            ...
            // No conversion necessary
                ...
                if (log.isDebugLevel()) {
                    log.debug(TAG, "    No conversion required, value is already a "
                                    + toString(targetType));
                }
                return value;
            ...
            // Convert --> Type
                ...
                if (log.isDebugLevel()) {
                    log.debug(TAG, "    Converted to " + toString(targetType) +
                                   " value '" + result + "'");
                }
                return result;
            }
    ...
    protected Object handleError(Class type, Object value, Throwable cause) {
        if (log.isDebugLevel()) {
            if (cause instanceof ConversionException) {
                log.debug(TAG, "    Conversion threw ConversionException: " + cause.getMessage());
            } else {
                log.debug(TAG, "    Conversion threw " + cause);
            }
        }
        ...
        if (cause instanceof ConversionException) {
            ...
            if (log.isDebugLevel()) {
                log.debug(TAG, "    Re-throwing ConversionException: " + cex.getMessage());
                log.debug(TAG, "    " + DEFAULT_CONFIG_MSG);
            }
        } else {
            ...
            if (log.isDebugLevel()) {
                log.debug(TAG, "    Throwing ConversionException: " + msg);
                log.debug(TAG, "    " + DEFAULT_CONFIG_MSG);
            }
//            BeanUtils.initCause(cex, cause);
    ...
    protected Object handleMissing(Class type) {
        ...
        if (useDefault || type.equals(String.class)) {
            ...
            if (useDefault && value != null && !(type.equals(value.getClass()))) {
                try {
                    ...
                    log.error(TAG, "    Default conversion to " + toString(type)
                            + "failed: " + t);
                }
            }
            if (log.isDebugLevel()) {
                log.debug(TAG, "    Using default "
        ...
        ConversionException cex =  new ConversionException("No value specified for '" +
                toString(type) + "'");
        if (log.isDebugLevel()) {
            log.debug(TAG, "    Throwing ConversionException: " + cex.getMessage());
            log.debug(TAG, "    " + DEFAULT_CONFIG_MSG);
        }
        throw cex;
    ...
    protected void setDefaultValue(Object defaultValue) {
        useDefault = false;
        if (log.isDebugLevel()) {
            log.debug(TAG, "Setting default value: " + defaultValue);
        }
