...
public abstract class AbstractSearchAutoBindMMActivity<CRITERIAITEM extends MIdentifiable, RESULTITEM extends MIdentifiable> extends
	...
	protected void initCriteriaComponent( View p_oCriteriaView, int p_iKeyView ) {
		...
		this.criteriaComponent.getComponentFwkDelegate().setDescriptor(VisualComponentDescriptorsHandler.getInstance().getRealDescriptor(
			this.criteriaComponent.getComponentFwkDelegate().getName()));
		this.criteriaComponent.setViewModel(this.criteriaViewModel);
		this.criteriaComponent.getComponentFwkDelegate().getDescriptor().inflate(String.valueOf(p_iKeyView), this.criteriaComponent, null);
...
//			Method oOnReloadResultsMethod = this.getClass().getMethod("doOnReloadResults", ListenerOnDataLoaderReloadEvent.class);
			...
			this.getAndroidApplication().addBusinessEventListener(this.getSelectedItemEventClass(), this, null);
			...
			this.getAndroidApplication().getClassAnalyser().getClassAnalyse(this).addBusinessEventListener(
					this.getSelectedItemEventClass(), this.getClass(), oOnNewSelectedItemMethod, null );
...
//					this.getClass(), oOnReloadResultsMethod);
	...
	protected void dismissCriteriaDialog() {
		if ( this.isCriteriaDialogVisible && this.criteriaDialog.getDialog().isShowing()) {
			...
			this.criteriaComponent.getComponentFwkDelegate().getDescriptor().unInflate(String.valueOf(this.criteriaDialog.hashCode()), this.criteriaComponent, null);
		}
	}
