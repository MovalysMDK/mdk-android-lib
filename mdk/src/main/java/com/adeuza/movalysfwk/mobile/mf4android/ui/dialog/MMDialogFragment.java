...
public class MMDialogFragment extends DialogFragment {
	...
	public void show(FragmentManager p_oManager, String p_sTag) {
		Fragment oPrevDialog = p_oManager.findFragmentByTag(p_sTag);
		FragmentTransaction ft = p_oManager.beginTransaction();
		if ( oPrevDialog != null ) {
			ft.remove(oPrevDialog);
		}
		ft.add(this, p_sTag);
	...
	private void setTagFragmentOnView(View p_oView) {
		...
		if(((ConfigurableVisualComponent)p_oView).getComponentFwkDelegate().getConfiguration() !=null) {
				((ConfigurableVisualComponent)p_oView).getComponentFwkDelegate().setFragmentTag(getFragmentDialogTag());
			}
		}
	...
	public void setDismissListener(WeakReference<OnDismissListener> p_oDismissListener) {
