...
import com.adeuza.movalysfwk.mobile.mf4android.actiontask.ActionListenerHandler;
...
public class AndroidApplication extends Application {
	...
	/** generated layout prefix */
	private static final String MDK_LAYOUT_SUFFIX = "g";

	private static final int ACCESS_NETWORK_STATE_PERMISSION = 0;
	...
	private ActionListenerHandler actionListenerHandler = new ActionListenerHandler();
	...
	public static AndroidApplication getInstance() {
		return (AndroidApplication) Application.getInstance();
	}
	...
	/**
	 * Returns the instance of the currently displayed activity
	 * @return the currently displayed activity
	 */
	...
	public void androidPreInit(String p_sMainPackage) {
		...
		if (androidIdInt2String==null) {
			...
			// TODO : a améliorer
			try {
				Class<?> oWksMasterDetail = Class.forName("com.adeuza.movalysfwk.mobile.mf4android.ui.views.MMWorkspaceMasterDetailFragmentLayout");
				Class<?> oWksConfLoader = Class.forName("com.adeuza.movalysfwk.mobile.mf4android.configuration.loader.WorkspaceFragmentConfigurationLoader");
				this.configurationLoaders.put(oWksMasterDetail.getName(), (ConfigurationLoader) oWksConfLoader.newInstance());
			} catch (ClassNotFoundException | InstantiationException | IllegalAccessException | NoClassDefFoundError e) {
				Log.d("androidPreInit", e.getMessage());
			}
	...
	public void initializeScreensDescriptor() {
		...
		try {
			...
			for (Field oField : androidRlayout.getFields()) {
				...
				sKey = oField.getName();
				// on ne traite que les layout générés (pas ceux présents dans l'apklib)
				if (!sKey.startsWith(MDK_LAYOUT_SUFFIX)) {
					continue;
				}
				...
				oMasterDescriptor = new TheoriticalVisualComponentDescriptor(sKey, null);
				...
				// si le descriptor possède un seul fils et que 
				// - celui ci porte le même nom que son père le père ou (ce cas arrive dans le cas des "masters" pour les listes)
				// qu'il n'a lui même pas de fils (ex cas d'une cellule de spinner, avec un seul composant)
				// on renomme le descripteur avec le nom du fils
				if (oMasterDescriptor.getComponents().size() == 1
						&& (oMasterDescriptor.getComponents().get(0).getName().equals(oMasterDescriptor.getName())
						|| oMasterDescriptor.getComponents().get(0).getComponents().isEmpty())) {
					oMasterDescriptor = oMasterDescriptor.getComponents().get(0);
					oMasterDescriptor.setMaster(true);
				}
				
				oMasterDescriptor.setLayoutFile(sKey);
	...
	public InputStream openInputStream(MContext p_oContext, ApplicationR p_oResourceKey) {
		return ((AndroidMMContext) p_oContext).getAndroidNativeContext().getResources()
	...
	/**
	 * Returns an android resource identifier from its name in a given {@link ApplicationRGroup}
	 * @param p_sKey the {@link ApplicationRGroup} to look for the element
	 * @param p_sElementKey the element name
	 * @return the id of the element, or -1 if it could not be found
	 */
	...
	/**
	 * Returns an android resource identifier from its {@link ApplicationR}
	 * @param p_sRKey the {@link ApplicationR} resource to look for
	 * @return the id of the element, or -1 if it could not be found
	 */
	...
	public String getDecriptedStringSetting(final String p_sKey) {
		...
		return LocalAuthHelperImpl.getInstance().decrypt(this.getApplicationContext(), encryptedString);
	}
	 ...
	 * <p>
	 * This method must be call inside an action requering ACCESS_NETWORK_STATE permission.
	 * </p>
	...
	public void analyzeClassOf(Object p_oObject ){ 
		...
		Map<Class<?>, List<Class<? extends BusinessEvent<?>>>> oBusinessEvents = oAnalyse.getBusinessEventForObject(p_oObject.getClass());
		if (oBusinessEvents!=null) {
			for(Entry<Class<?>, List<Class<? extends BusinessEvent<?>>>> oBusinessEventEntry : oBusinessEvents.entrySet()) {
				for (Class<? extends BusinessEvent<?>> oBusinessEventClass : oBusinessEventEntry.getValue()) {
					addBusinessEventListener(oBusinessEventClass, p_oObject, oBusinessEventEntry.getKey());
				}
			}
		}
	 ...
	 * @param p_oTargetComponent the component launching the voice recognition
	 */
	...
	/**
	 * Sets the screen dictionnary
	 * @param p_oScreenDictionary the dictionnary to set
	 */
	...
	/**
	 * Starts an activity with an intended result
	 * @param p_oIntent the intent launching the activity
	 * @param p_iRequestCode the result code wanted
	 */
	...
	public void addActiveDisplayList(ListenerIdentifier p_oListenerIdentifier) {
		super.addActiveDisplayList(p_oListenerIdentifier);
		this.actionListenerHandler.invokePendingEvents(p_oListenerIdentifier, 
			BeanLoader.getInstance().getBean(MContextFactory.class).createContext());
	}
	 ...
	 * and finally kill application process.
	...
	/**
	 * Returns the {@link ActionListenerHandler} hold by the class
	 * @return the {@link ActionListenerHandler}
	 */
	public ActionListenerHandler getActionListenerHandler() {
		return this.actionListenerHandler;
	}

	@Override
	public boolean displayMandatorySettingWindow() {
		return String.valueOf(true).equals(getStringResource(AndroidApplicationR.display_setting_window));
	}
	
	@Override
	public boolean isNotMantatoryRequested(String p_sPermission) {
		String[] oNotMantatoryRequested = this.getCurrentActivity().getResources().getStringArray(this.getAndroidIdByRKey(AndroidApplicationR.fwk_not_mandatory_permission));
		return Arrays.asList(oNotMantatoryRequested).contains(p_sPermission);
	}
