...
/**
 * <p>TODO Décrire la classe MMCheckBoxPreference</p>
 *
 *
 *
 */

public class MMCheckBoxPreference extends CheckBoxPreference {
	/**
	 * TODO Décrire le constructeur MMPreferenceCategory
	 * @param p_oContext
	 */
	public MMCheckBoxPreference(Context p_oContext) {
		super(p_oContext);
	}
	 ...
	 * @param p_oAttrs
	 */
	public MMCheckBoxPreference(Context p_oContext, AttributeSet p_oAttrs) {
	 ...
	 * @param p_oDefStyle
	 */
	public MMCheckBoxPreference(Context p_oContext, AttributeSet p_oAttrs, int p_oDefStyle) {
	...
	protected final void init(final AttributeSet p_oAttrs) {
		...
		final int iSummaryOffId = p_oAttrs.getAttributeResourceValue(AndroidApplication.ANDROID_XML_NAMESPACE, "summaryOff", 0);
		if (iSummaryOffId != 0) {
			this.setSummaryOff(((AndroidApplication)Application.getInstance()).getStringResource(iSummaryOffId));
		}

		final int iSummaryOnId = p_oAttrs.getAttributeResourceValue(AndroidApplication.ANDROID_XML_NAMESPACE, "summaryOn", 0);
		if (iSummaryOnId != 0) {
			this.setSummaryOn(((AndroidApplication)Application.getInstance()).getStringResource(iSummaryOnId));
		}
	}
}
