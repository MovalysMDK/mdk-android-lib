...
public class SqlInValueCondition extends AbstractSqlInCondition implements Cloneable {
	...
	public static SqlInValueCondition createSqlInValueCondition(List<SqlInField> p_listFields, List<?> p_listValues) {
		return new SqlInValueCondition(p_listFields, p_listValues);
	}

	/**
	 * Nombre de valeurs à ajouter pour compléter le dernier IN
	 */
	private int nbValueToAdd = 0; 
	 ...
	 */
	public SqlInValueCondition(String p_sFieldName, String p_sAlias, @SqlType int p_iSqlType, List<?> p_listValues ) {
		setField(p_sFieldName, p_sAlias, p_iSqlType);
	 ...
	 * @param p_listValues liste des valeurs
	 */
	public SqlInValueCondition(Field p_oField, String p_sAlias, @SqlType int p_iSqlType, List<?> p_listValues ) {
		setField(p_oField, p_sAlias, p_iSqlType);
		this.values = p_listValues;
	}
	 ...
	 * @param p_listValues liste des valeurs
	 */
	private SqlInValueCondition(List<SqlInField> p_listFields, List<?> p_listValues) {
	 ...
	 * @param p_listValues liste des valeurs
	 */
	public SqlInValueCondition(FieldType[] p_listFields, String p_sAlias, List<?> p_listValues) {
		for( FieldType oPairValue: p_listFields) {
			this.getListFields().add( new SqlInField(oPairValue.getField().name(), p_sAlias, oPairValue.getType()));
		}
		this.values = p_listValues;
	}
	 ...
	 * @param p_listValues liste des valeurs
	 */
	public SqlInValueCondition(FieldType[] p_listFields, List<?> p_listValues) {
		for( FieldType oPairValue: p_listFields) {
			this.getListFields().add( new SqlInField(oPairValue.getField().name(), oPairValue.getType()));
		}
		this.values = p_listValues;
	}
	...
	private void toSql(StringBuilder p_oSql, boolean p_bFieldNameWithAlias) {
		if (!this.values.isEmpty()) {
			...
			this.nbValueToAdd = IN_CLAUSE_PREFERRED_SIZE - iNbTupleInClause;
			// On ajoute le padding (avec la dernière valeur de la liste) pour obtenir un IN multiple de IN_CLAUSE_PREFERRED_SIZE
			for (int iIndexValueToAdd = 0; iIndexValueToAdd < (this.nbValueToAdd); iIndexValueToAdd++) {
				p_oSql.append(',');
				p_oSql.append(sFieldValue);
			}
			if (!bFirst) {
	...
	public void bindValues(StatementBinder p_oStatementBinder) throws DaoException {
		if(!this.values.isEmpty()){
			...
			for (int iCpt = 1; iCpt <= this.nbValueToAdd; iCpt++) {
				int iIndex = 0;
				for (int iRank=0 ; iRank<getListFields().size() ; iRank++) {
	...
	public AbstractSqlCondition clone() {
		...
		AbstractSqlInCondition r_oSqlCondition = createSqlInValueCondition(listSqlInFields, this.values);
