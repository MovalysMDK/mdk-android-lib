...
public abstract class AbstractFwkControllerImpl extends AbstractBaseFwkControllerImpl {
	...
	protected ActionParameter launchAction(MContext p_oContext, Notifier p_oNotifier, @SuppressWarnings("rawtypes") 
		...
		if (p_oContext instanceof MExtendedContextImpl) {
			oLaunchParameter = ((MExtendedContextImpl) p_oContext).getLaunchParameter();
		}
	...
	public void doOnAsyncActionEnd(long p_lActionId, ActionHandler... p_oActions) throws ActionException {
		...
		if (oHandler.getContext() instanceof MExtendedContextImpl) {
			LaunchParameter oLp = new LaunchParameter();
			((MExtendedContextImpl) oHandler.getContext()).setLaunchParameter(oLp);
		}
		if (p_oActions == null) {
	...
	public ActionTaskOut launchActionByActionTask(MMActionTask p_oTask) throws ActionException {
		...
		oLp.setTask(p_oTask);
		((MExtendedContextImpl) p_oTask.getContext()).setLaunchParameter(oLp);
	...
	protected ActionParameter launchActionWithBusyVerification(final Notifier p_oNotifier, @SuppressWarnings("rawtypes")// pas moyen
		...
		Application.getInstance().getLog().info(Application.LOG_TAG, "LaunchActionWithBusyVerification: launch action="+p_oAction.toString());
		ActionParameter r_oResult = null;
		if (canBeginAction(p_oAction) || AlertAction.class.isAssignableFrom(p_oAction.getClass())) {
			...
			try {
				...
				if (oContext instanceof MExtendedContextImpl) {
					LaunchParameter oLp = new LaunchParameter();
					((MExtendedContextImpl) oContext).setLaunchParameter(oLp);
				}
				r_oResult = launchAction(oContext, p_oNotifier, p_oAction, p_oParameterIn);
			}
			catch (Exception e) {
				Application.getInstance().getLog().error(Application.LOG_TAG, "FwkControllerImpl: Exception in launchActionWithBusyVerification",e);
				oContext.getMessages().addMessage(ExtFwkErrors.LaunchActionError);
				Application.getInstance().getLog().error(Application.LOG_TAG, "Error: " + p_oAction.getClass().getName(), e);
			}
			finally {
				Application.getInstance().getLog().info(Application.LOG_TAG, "LaunchActionWithBusyVerification: release action="+p_oAction.toString());
			...
			Application.getInstance().getLog().error(Application.LOG_TAG, "LaunchActionWithBusyVerification> conflit d'action : "+p_oAction.toString());
	...
	public void releaseBusyStatus(MContext p_oContext, Action<?,?,?,?> p_oAction) {
		if (this.busy && !p_oAction.isConcurrentAction()) {
			synchronized (oBusy) {
				Application.getInstance().getLog().info(Application.LOG_TAG, "this.busy = false, action class: " + p_oAction.getClass().getName());
				this.busy = false;
			}
		}
		synchronized (oRunning) {
			this.running--;
		}

		if (p_oContext instanceof MExtendedContextImpl) {
			MExtendedContextImpl exContext = (MExtendedContextImpl)p_oContext;
			if (exContext.getLaunchParameter()!=null && exContext.getLaunchParameter().isUseActionTask()) {
				synchronized (this.actionTaskQueue) {
					MMActionTask oTask = this.actionTaskQueue.peek();
					if (oTask != null && this.canBeginAction(oTask.getAction())) {
						this.actionTaskQueue.remove();
						Application.getInstance().getLog().info(Application.LOG_TAG, "Lancement de " + oTask.getAction().getClass().getName());
						oTask.execAction();
					}
				}
			}
	...
	protected void invokeEventMethod(MContext p_oContext, Method p_oMethod, Object p_oEvent, Object p_oListener) {
		if (p_oContext instanceof MExtendedContextImpl && ((MExtendedContextImpl)p_oContext).isLaunchedByActionTask()) {
			...
			((MExtendedContextImpl)p_oContext).getLaunchParameter().getTask().publishActionProgress(oProgress);
		}
		else {
			try {
				p_oMethod.invoke(p_oListener, p_oEvent);
			} catch (IllegalArgumentException e) {
				Application.getInstance().getLog().error(Application.LOG_TAG, "FwkControllerImpl: Impossible d'évoquer une méthode", e);
