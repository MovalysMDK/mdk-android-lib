...
public class CascadeOptimDefinition {
	 ...
	 */
	public CascadeOptimDefinition( PairValue<Field, SqlType>[] p_oPkFields, String p_sAlias ) {
		
		this.cascadeFkFields = new HashMap<ICascade, List<SqlInField>>(MAX_FK_NUMBER);		
		this.pkFields = new ArrayList<SqlInField>();
		for( PairValue<Field, SqlType> oPkField : p_oPkFields ) {
			this.pkFields.add(new SqlInField(oPkField.getKey().toString(),p_sAlias,oPkField.getValue()));
		}
	...
	public void registerCascade(ICascade p_oCascade,
			PairValue<Field, SqlType>[] p_oFkFields, String p_sAlias) {
		
		List<SqlInField> listFkInFields = new ArrayList<SqlInField>();
		for( PairValue<Field, SqlType> oFkField : p_oFkFields ) {
			listFkInFields.add(new SqlInField(oFkField.getKey().toString(),p_sAlias,oFkField.getValue()));
		}
