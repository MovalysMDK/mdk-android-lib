...
public abstract class AbstractSqlJoin {
	 ...
	 */
	private List<AbstractSqlCondition> onConditions = new ArrayList<AbstractSqlCondition>();
	...
	public void addOnCondition( AbstractSqlCondition p_oSqlCondition ) {
		this.onConditions.add(p_oSqlCondition);
	}
	...
	public void toSql( StringBuilder p_oSql, ToSqlContext p_oToSqlContext) throws DaoException {
		...
		if ( !onConditions.isEmpty()) {
			...
			sConditionBuilder.append(' ');
			for( AbstractSqlCondition oSqlCondition : onConditions ) {
				if ( bFirst ) {
					bFirst = false ;
				} else {
					sConditionBuilder.append(' ');
					sConditionBuilder.append(oSqlCondition.getLink());
					sConditionBuilder.append(' ');
				}
				oSqlCondition.toSql(sConditionBuilder, p_oToSqlContext);
			}
		}
	 ...
	 * @throws java.sql.SQLException echec du binding des valeurs
	...
	public void bindValues(StatementBinder p_oStatementBinder) throws SQLException {
		for( AbstractSqlCondition oSqlCondition: this.onConditions) {
	...
	/**
	 * Retourne les conditions de jointures
	 *
	 * @return conditions de jointure
	 */
	protected List<AbstractSqlCondition> getConditions() {
