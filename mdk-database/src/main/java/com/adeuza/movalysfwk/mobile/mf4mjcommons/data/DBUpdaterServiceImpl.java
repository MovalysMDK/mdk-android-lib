...
public class DBUpdaterServiceImpl implements DBUpdaterService {
	...
	private void executeInstructions(MContext p_oContext, List<String> p_oInstructions) throws ServiceException {
		Transaction oTransaction = ((ItfTransactionalContext)p_oContext).getTransaction();
		try {
			Connection oConnection = oTransaction.getConnection();
			...
			throw new ServiceException("Database creation failed", oSqlException);
		}
	}
