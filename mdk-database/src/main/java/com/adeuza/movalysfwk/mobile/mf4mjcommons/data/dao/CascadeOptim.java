...
public class CascadeOptim {
		...
		if ( !listFkValue.isEmpty()) {
			...
			for (int iCpt = 1; iCpt <= iNbKey; iCpt++) {
				...
				if (iCpt % IN_SIZE == 0 || iCpt == iNbKey) {
					SqlInValueCondition oInClause = new SqlInValueCondition(listFkFields,
							listValueOfIn);
	...
	public List<SqlInValueCondition> getInClausesForMainEntities() {
		...
		if ( !this.pkValues.isEmpty()) {
			...
			for (int iCpt = 1; iCpt <= iNbKey; iCpt++) {
				...
				// si restriction atteinte ou si dernière entité
				if (iCpt % IN_SIZE == 0 || iCpt == iNbKey) {
					SqlInValueCondition oInClause = new SqlInValueCondition(this.cascadeOptimDefinition.getPkFields(), listValueOfIn);
					r_listIn.add(oInClause);
					listValueOfIn = new ArrayList<Object>();
				}
			}
	 ...
	 */
	public List<SqlInValueCondition> getInClausesForJoinClasses(ICascade p_oCascade, PairValue<Field, SqlType>[] p_oDestEntityPK,
			String p_sAlias ) {
		...
		for( PairValue<Field, SqlType> oDestEntityPK : p_oDestEntityPK) {
			listSqlInFields.add( new SqlInField(oDestEntityPK.getKey().toString(), p_sAlias, oDestEntityPK.getValue()));
		}
		...
		if ( !listFkValue.isEmpty()) {
			...
			for (int iCpt = 1; iCpt <= iNbKey; iCpt++) {
				...
				// si restriction atteinte ou si dernière entité
				if (iCpt % IN_SIZE == 0 || iCpt == iNbKey) {
					SqlInValueCondition oInClause = new SqlInValueCondition(listSqlInFields,listValueOfIn);
					r_listIn.add(oInClause);
					listValueOfIn = new ArrayList<Object>();
				}
			}
